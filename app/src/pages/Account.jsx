...
const Account = () => {
  ...
  const handleAccount = async (e) => {
    ...
    try {
      ...
      const res = await Axios.put(`/users/edit/${id}`, values, {
        headers: {
          'Authorization': `Bearer ${token}`,
        }
      });
      ...
      <div className="grid md:grid-cols-3 h-screen">
        ...
        <div className="md:col-span-2 w-full grid place-content-center">
          ...
          <form action="" className="flex flex-col gap-5 w-[300px] p-5 md:w-[500px]" onSubmit={handleAccount}>
            ...
            <input type="text" value={values.fullname} name="fullname" id="fullname" onChange={(e) => setValues({...values, fullname: e.target.value})} className="border border-green-600 outline-none py-2 px-5 rounded" placeholder="Enter Fullname" />
            <input type="text" value={values.username} name="username" id="username" onChange={(e) => setValues({...values, username: e.target.value})} className="border border-green-600 outline-none py-2 px-5 rounded" />
            <input type="email" value={values.email} name="email" id="email" onChange={(e) => setValues({...values, email: e.target.value})} className="border border-green-600 outline-none py-2 px-5 rounded" />
